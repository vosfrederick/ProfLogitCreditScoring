...
from .rga import RGA
from .empc import EMPCS, EMPChurn
from .utils import default_formula
from .utils import FLOAT_TYPES, NUM_TYPE


class ProfLogitBase(object):
    """
    ProfLogit for Credit Scoring
    =======================================

    Maximizing empirical EMPCS by optimizing
    ...
    def fit(self, X, y, churn=False):
        ...
        y : 1D array-like or label indicator array, shape=(n_samples,)
            Binary target values. Defaulters have a ``case_label`` value.
        ...
        # Init
        self.emp = EMPCS(y_true=y, **self.empc_kws)
    ...
    def score(self, X, y_true):
        """
        Compute EMPCS performance score.
        ...
        empc = EMPCS(y_true, y_score, **self.empc_kws).empc()
...
def proflogit_fobj(theta, *args):
    ...
    # emp: (EMPCS) object to compute EMPCS
...
class ProfLogitCCP(object):
    ...
    def score(self, X, y_true, churn=False):
        """
...
class ProfLogitCS(ProfLogitBase):
    """
    ProfLogit for credit scoring
    ===================

    This is a convenient wrapper for ProfLogitCS to make use of
    patsy's powerful ``formula`` functionality that allows building
    appropriate design matrices easily.
