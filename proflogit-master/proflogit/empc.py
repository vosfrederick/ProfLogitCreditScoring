...
class EMPCS(object):
    ...
    ----------
    y_true : 1D array-like or label indicator array, shape=(n_samples,)
        Binary target values. Defaulters have a ``cases_label`` value.
    ...
    p0 : float
        The probability that Lambda will get a value of 0 or that the customer pays back the entire loan.

    p1 : float
        The probability that Lambda gets a value of 1 or that customer defaults the loan entirely.

    ROI : float
        Constant ROI per granted loan (percentage)

    case_label : str or int or float (default: 1)
        Label for the identification of cases (i.e., defaulters).

    Note
    ----
    The EMPCS requires that the defaulters are encoded as 0. However, this implementation assumes
    the standard notation ('default': 1, 'no default': 0) and makes the necessary
    changes internally. An equivalent R implementation is available in [2].
    ...
    def __init__(
        ...
        """Constructor for EMPCS"""
        assert isinstance(p0, NUM_TYPE) and 0 < p0 < 1
        assert isinstance(p1, NUM_TYPE) and 0 < p1 < 1
        assert isinstance(ROI, NUM_TYPE) and 0 < ROI < 1
        assert isinstance(case_label, (str, NUM_TYPE))

        self._yt = None
        self._ys = None
        self._lambdaa_needs_update = None
        y = np.asarray(y_true)  # "Sets y_true as an array"
        self.y_true = y == case_label  # Make it binary
        self.n_samples = len(self.y_true)
        if y_score is not None:
            self.y_score = np.asarray(y_score)
    ...
    def y_score(self, value):
        ...
        if value is not None:
            y_score = np.asarray(value)
            assert y_score.ndim == 1
            self._lambdaa_needs_update = True
            self._ys = y_score
    ...
    def empc(self, y_score=None):
        ...
        -------
        empc : float
            Empirical EMP estimate for Credit Scoring prediction.

        """
        if y_score is not None:
            self.y_score = y_score
        # assert self._lambdaa_needs_update is not None and self._ys is not None
        assert len(self._yt) == len(self._ys)

        # Lambdaa has been changed, empc needs to be recomputed
        # assert self._lambdaa_needs_update
        self._empc = self._compute_empc()
        return self._empc
    ...
    def _compute_empc(self):
        ...
        lambdaaii = self._lambdaa[:-1]
        lambdaaie = self._lambdaa[1:]
        F0 = self._F0[:len(lambdaaii)]
        F1 = self._F1[:len(lambdaaii)]
        alpha_ = 1 - self.p0 - self.p1

        empc = np.sum(alpha_ * (lambdaaie - lambdaaii) * (
                    self.pi0 * F0 * (lambdaaie + lambdaaii) / 2 - self.ROI * self.pi1 * F1)) + \
               (self.pi0 * F0[-1:] - self.ROI * self.pi1 * F1[-1:] * self.pi1) * self.p1
    ...
    def _compute_lambda(self):
        ...
        with warnings.catch_warnings():
            # Ignore RuntimeWarning: division by zero
            # It is taken care of in the next lambda line
            warnings.simplefilter("ignore")

            expression = (self.pi1 * self.ROI / self.pi0) * np.diff(self._F1) / np.diff(self._F0)
            lambdaa = np.append([0], expression)
        self._lambdaa = np.append(lambdaa[lambdaa < 1], [1])
        self._lambdaa_needs_update = False
